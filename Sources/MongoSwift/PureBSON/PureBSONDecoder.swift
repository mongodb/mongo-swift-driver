...
public class PureBSONDecoder {
    ...
    public enum DfateDecodingStrategy {
    ...
    ///
    /// - SeeAlso: bsonspec.org
    public enum UUfIDDecodingStrategy {
    ...
    ///
    /// - SeeAlso: bsonspec.org
    public enum DatfaDecodingStrategy {
    ...
    public var dateDecodingStrategy: BSONDecoder.DateDecodingStrategy = .bsonDateTime

    /// The strategy used for decoding `UUID`s with this instance.
    public var uuidDecodingStrategy: BSONDecoder.UUIDDecodingStrategy = .binary

    /// The strategy used for decoding `Data`s with this instance.
    public var dataDecodingStrategy: BSONDecoder.DataDecodingStrategy = .binary

    /// Options set on the top-level decoder to pass down the decoding hierarchy.
    internal struct _Options {
        internal let userInfo: [CodingUserInfoKey: Any]
        internal let dateDecodingStrategy: BSONDecoder.DateDecodingStrategy
        internal let uuidDecodingStrategy: BSONDecoder.UUIDDecodingStrategy
        internal let dataDecodingStrategy: BSONDecoder.DataDecodingStrategy
    }
     ...
     */
    public func decode<T: Decodable>(_ type: T.Type, from document: PureBSONDocument) throws -> T {
        // if the requested type is `Document` we're done
        if let doc = document as? T {
            return doc
        }
        let _decoder = _PureBSONDecoder(referencing: document, options: self.options)
...
//        }
    ...
    internal func internalDecode<T: Decodable>(
            _ type: T.Type,
            from document: PureBSONDocument,
...
internal class _PureBSONDecoder: Decoder {
    /// The decoder's storage.
    internal var storage: _PureBSONDecodingStorage

    /// Options set on the top-level decoder.
    internal let options: PureBSONDecoder._Options
    ...
    fileprivate init(referencing container: PureBSONValue,
                     at codingPath: [CodingKey] = [],
                     options: PureBSONDecoder._Options) {
        self.storage = _PureBSONDecodingStorage()
    ...
    public func container<Key>(keyedBy type: Key.Type) throws -> KeyedDecodingContainer<Key> {
        guard let topContainer = self.storage.topContainer as? PureBSONDocument else {
            throw DecodingError._PuretypeMismatch(at: self.codingPath,
                                              expectation: PureBSONDocument.self,
                                              reality: self.storage.topContainer)
        }

        let container = _PureBSONKeyedDecodingContainer<Key>(referencing: self, wrapping: topContainer)
    ...
    public func unkeyedContainer() throws -> UnkeyedDecodingContainer {
        ...
        return _PureBSONUnkeyedDecodingContainer(referencing: self, wrapping: arr)
    }
}

// Storage for a _PureBSONDecoder.
internal struct _PureBSONDecodingStorage {
    /// The container stack, consisting of `BSONValue`s.
    fileprivate private(set) var containers: [PureBSONValue] = []
    ...
    internal var topContainer: PureBSONValue {
...
extension _PureBSONDecoder {
    fileprivate func unboxBSONValue<T: PureBSONValue>(_ value: PureBSONValue, as type: T.Type) throws -> T {
        ...
        guard let typed = value as? T else {
            throw DecodingError._PuretypeMismatch(at: self.codingPath, expectation: type, reality: value)
        }
        return typed
    }

    fileprivate func unboxNumber<T: PureCodableNumber>(_ value: PureBSONValue, as type: T.Type) throws -> T {
        guard let primitive = T(from: value) else {
            throw DecodingError._PurenumberMismatch(at: self.codingPath, expectation: type, reality: value)
        }
        return primitive
    }

    fileprivate func unboxData(_ value: PureBSONValue) throws -> Data {
        ...
        case .binary:
            let binary = try self.unboxBSONValue(value, as: PureBSONBinary.self)
            return binary.data
        case.base64:
    ...
    fileprivate func unboxDate(_ value: PureBSONValue) throws -> Date {
    ...
    fileprivate func unboxUUID(_ value: PureBSONValue) throws -> UUID {
    ...
    fileprivate func unbox<T: Decodable>(_ value: PureBSONValue, as type: T.Type) throws -> T {
...
private struct _PureBSONKeyedDecodingContainer<K: CodingKey>: KeyedDecodingContainerProtocol {
    typealias Key = K

    /// A reference to the decoder we're reading from.
    private let decoder: _PureBSONDecoder

    /// A reference to the container we're reading from.
    fileprivate let container: PureBSONDocument

    /// The path of coding keys taken to get to this point in decoding.
    public private(set) var codingPath: [CodingKey]

    /// Initializes `self`, referencing the given decoder and container.
    fileprivate init(referencing decoder: _PureBSONDecoder, wrapping container: PureBSONDocument) {
    ...
    private func getValue(forKey key: Key) throws -> PureBSONValue {
        ...
        return entry.bsonValue
    }

    /// Decode a BSONValue type from this container for the given key.
    private func decodeBSONType<T: PureBSONValue>(_ type: T.Type, forKey key: Key) throws -> T {
    ...
    /// Decodes a PureCodableNumber type from this container for the given key.
    private func decodeNumber<T: PureCodableNumber>(_ type: T.Type, forKey key: Key) throws -> T {
        let entry = try getValue(forKey: key)
        return try self.decoder.with(pushedKey: key) {
    ...
    public func decodeNil(forKey key: Key) throws -> Bool {
        ...
        return self.container[key.stringValue] == BSON.null
    }
    ...
    public func nestedContainer<NestedKey>(keyedBy type: NestedKey.Type,
        ...
        return try self.decoder.with(pushedKey: key) {
            ...
            guard let doc = value as? PureBSONDocument else {
                throw DecodingError._PuretypeMismatch(at: self.codingPath, expectation: PureBSONDocument.self, reality: value)
            }

            let container = _PureBSONKeyedDecodingContainer<NestedKey>(referencing: self.decoder, wrapping: doc)
            return KeyedDecodingContainer(container)
        }
    }
    ...
    private func _superDecoder(forKey key: CodingKey) throws -> Decoder {
        return try self.decoder.with(pushedKey: key) {
            ...
            return _PureBSONDecoder(referencing: value.bsonValue, at: self.decoder.codingPath, options: self.decoder.options)
        }
    }
...
private struct _PureBSONUnkeyedDecodingContainer: UnkeyedDecodingContainer {
    /// A reference to the decoder we're reading from.
    private let decoder: _PureBSONDecoder

    /// A reference to the container we're reading from.
    private let container: [PureBSONValue]
    ...
    private mutating func decodeBSONType<T: PureBSONValue>(_ type: T.Type) throws -> T {
        try self.checkAtEnd()
    ...
    private mutating func decodeNumber<T: PureCodableNumber>(_ type: T.Type) throws -> T {
        try self.checkAtEnd()
        return try self.decoder.with(pushedKey: _BSONKey(index: self.currentIndex)) {
    ...
    public mutating func nestedContainer<NestedKey: CodingKey>(keyedBy type: NestedKey.Type)
        ...
        return try self.decoder.with(pushedKey: _BSONKey(index: self.currentIndex)) {
            try self.checkAtEnd()
            let doc = try self.decodeBSONType(PureBSONDocument.self)
            self.currentIndex += 1
            let container = _PureBSONKeyedDecodingContainer<NestedKey>(referencing: self.decoder, wrapping: doc)
            return KeyedDecodingContainer(container)
        }
...
//        }
    ...
    public mutating func superDecoder() throws -> Decoder {
        return try self.decoder.with(pushedKey: _BSONKey(index: self.currentIndex)) {
            ...
            self.currentIndex += 1
            return _PureBSONDecoder(referencing: value, at: self.decoder.codingPath, options: self.decoder.options)
        }
    }
}

/// :nodoc:
extension _PureBSONDecoder: SingleValueDecodingContainer {
    ...
    private func decodeBSONType<T: PureBSONValue>(_ type: T.Type) throws -> T {
        try expectNonNull(T.self)
        return try self.unboxBSONValue(self.storage.topContainer, as: T.self)
    }

    /// Decode a CodableNumber type from this container.
    private func decodeNumber<T: PureCodableNumber>(_ type: T.Type) throws -> T {
        try expectNonNull(T.self)
...
internal struct _PureBSONKey: CodingKey {
    ...
    // swiftlint:disable:next force_unwrapping
    internal static let `super` = _PureBSONKey(stringValue: "super")! // this initializer never actually returns nil.
}

extension DecodingError {
    internal static func _PuretypeMismatch(at path: [CodingKey],
                                       expectation: Any.Type,
                                       reality: PureBSONValue) -> DecodingError {
        let description = "Expected to decode \(expectation) but found \(type(of: reality)) instead."
        return .typeMismatch(expectation, Context(codingPath: path, debugDescription: description))
    }

    internal static func _PurenumberMismatch(at path: [CodingKey],
                                         expectation: Any.Type,
                                         reality: PureBSONValue) -> DecodingError {
