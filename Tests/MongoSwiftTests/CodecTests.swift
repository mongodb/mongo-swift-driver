...
final class CodecTests: MongoSwiftTestCase {
    ...
    func testEncodingNonBSONNumbers() throws {
        ...
        let doc1: Document = [
            "int8": int32, "int16": int32, "uint8": int32, "uint16": int32,
            "uint32": int32, "uint64": int32, "uint": int32, "float": 42.0
        ]
    ...
    func testBSONValues() throws {
        ...
        let extjson = """
        {
            ...
            "code" : { "$code" : "hi", "$scope" : { "x" : { "$numberLong": "1" } } },
            "int" : 1,
    ...
    func testDecodeScalars() throws {
        ...
        expect(try decoder.decode(
            CodeWithScope.self,
            from: "{\"$code\": \"hi\" }"
        )).to(equal(CodeWithScope(code: "hi")))
    ...
    func testAnyBSONValueIsBSONCodable() throws {
        ...
        ).x.value).to(bsonEqual(doc))

        // values wrapped in an `AnyBSONStruct`
        let double = 42.0
        expect(try decoder.decode(
            AnyBSONValue.self,
            from: "{\"$numberDouble\": \"42\"}"
        ).value).to(bsonEqual(double))
        ...
        expect(try decoder.decode(
            AnyBSONStruct.self,
            from: wrappedDouble.canonicalExtendedJSON
        ).x.value).to(bsonEqual(double))
        ...
        expect(try decoder.decode(
            AnyBSONStruct.self,
            from: wrappedString.canonicalExtendedJSON
        ).x.value).to(bsonEqual(string))
        ...
        expect(try decoder.decode(
            AnyBSONStruct.self,
            from: wrappedOid.canonicalExtendedJSON
        ).x.value).to(bsonEqual(oid))
        ...
        expect(try decoder.decode(
            AnyBSONStruct.self,
            from: wrappedBool.canonicalExtendedJSON
        ).x.value).to(bsonEqual(bool))
        ...
        expect(try decoder.decode(
            AnyBSONStruct.self,
            from: wrappedDate.canonicalExtendedJSON
        ).x.value).to(bsonEqual(date))
        ...
        expect(try decoder.decode(
            AnyBSONStruct.self,
            from: wrappedRegex
        ).x.value).to(bsonEqual(regex))
        expect(
            try decoder.decode(
                AnyBSONStruct.self,
                from: wrappedRegex.canonicalExtendedJSON
            ).x.value as? RegularExpression
        ).to(equal(regex))
        ...
        expect(try decoder.decode(
            AnyBSONStruct.self,
            from: wrappedCode
        ).x.value).to(bsonEqual(code))
        expect(try decoder.decode(
            AnyBSONStruct.self,
            from: wrappedCode.canonicalExtendedJSON
        ).x.value).to(bsonEqual(code))

        // int32
        let int32 = Int32(5)

        expect(try decoder.decode(AnyBSONValue.self, from: "{ \"$numberInt\" : \"5\" }").value).to(bsonEqual(int32))
        ...
        expect(try decoder.decode(
            AnyBSONStruct.self,
            from: wrappedInt32.canonicalExtendedJSON
        ).x.value).to(bsonEqual(int32)
        )

        // int
        let int = 5

        expect(try decoder.decode(AnyBSONValue.self, from: "{ \"$numberLong\" : \"5\" }").value).to(bsonEqual(int))
        ...
        expect(try decoder.decode(
            AnyBSONStruct.self,
            from: wrappedInt.canonicalExtendedJSON
        ).x.value).to(bsonEqual(int))
        ...
        expect(try decoder.decode(
            AnyBSONStruct.self,
            from: wrappedInt64.canonicalExtendedJSON
        ).x.value).to(bsonEqual(int64))
        ...
        expect(try decoder.decode(
            AnyBSONStruct.self,
            from: wrappedDecimal.canonicalExtendedJSON
        ).x.value).to(bsonEqual(decimal))
        ...
        expect(try decoder.decode(
            AnyBSONStruct.self,
            from: wrappedMaxKey.canonicalExtendedJSON
        ).x.value).to(bsonEqual(maxKey))
        ...
        expect(try decoder.decode(
            AnyBSONStruct.self,
            from: wrappedMinKey.canonicalExtendedJSON
        ).x.value).to(bsonEqual(minKey))
