...
protocol SpecTest {
    ...
    func run(client: MongoClient, db: MongoDatabase, collection: MongoCollection<Document>) throws
}

/// Default implementation of a test execution.
extension SpecTest {
    internal func run(client: MongoClient, db: MongoDatabase, collection: MongoCollection<Document>) throws {
...
enum TestOperationResult: Decodable, Equatable {
    ...
    /// Result of CRUD operations that return a subset of fields represented in a `BulkWriteResult` (e.g. `InsertOne`).
