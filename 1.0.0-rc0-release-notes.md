...
This change only affects `ChangeStream`s and tailable `MongoCursor`s. (See: [change streams](https://docs.mongodb.com/manual/changeStreams/), [tailable cursors](https://docs.mongodb.com/manual/core/tailable-cursors/).) (By default, cursors are *not* tailable.)
...
`MongoCursor` and `ChangeStream` now conform to [`LazySequenceProtocol`](https://developer.apple.com/documentation/swift/lazysequenceprotocol), which inherits from `Sequence` (which these types conformed to previously).
